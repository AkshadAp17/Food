...
import { IUser, OrderWithDetails } from '../shared/mongoSchemas.js';
...
class EmailService {
  ...
  constructor() {
    const config: EmailConfig = {
      ...
      auth: {
        ...
        pass: process.env.EMAIL_PASSWORD || process.env.GMAIL_APP_PASSWORD || 'your-app-password'
      }
    };
  ...
  async sendOTPEmail(email: string, otpCode: string, firstName: string): Promise<boolean> {
    try {
      const mailOptions = {
        from: `"FoodieExpress" <${process.env.EMAIL_USER || process.env.GMAIL_USER}>`,
        to: email,
